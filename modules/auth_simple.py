...
def _save_session_cookie(user_email, user_name, user_role, login_method):
    ...
    try:
        ...
        # 아직 초기화 안됨 → 다음 렌더에 재시도
        st.session_state['_pending_cookie'] = (user_email, user_name, user_role, login_method)
...
def check_authentication():
    if st.session_state.get('authenticated', False):
        # 로그인 직후 pending cookie 저장 재시도
        if '_pending_cookie' in st.session_state:
            pending = st.session_state.pop('_pending_cookie')
            try:
                _save_session_cookie(*pending)
            except Exception:
                st.session_state['_pending_cookie'] = pending
        return True
