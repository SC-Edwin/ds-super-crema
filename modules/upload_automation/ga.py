    ...
    with container:
        ...
        if cache_key not in st.session_state or reload_btn:
            try:
                ...
                # GoogleAdsException 상세 에러 추출
                detailed_err = gads._extract_google_ads_error(e)
                # 상세 디버깅 정보
                with st.expander("🔍 디버깅 정보", expanded=True):
                    ...
                    # GoogleAdsException 추가 정보
                    try:
                        from google.ads.googleads.errors import GoogleAdsException
                        if isinstance(e, GoogleAdsException):
                            st.markdown("**Google Ads API 에러 코드:**")
                            st.write(f"request_id: {e.request_id}")
                            for i, error in enumerate(e.failure.errors):
                                st.write(f"에러 #{i+1}:")
                                st.write(f"  error_code: {error.error_code}")
                    ...
                    try:
                        ...
                        lid_raw = cfg.get('login_customer_id', 'N/A')
                        cid_raw = cfg.get('customer_id', 'N/A')
                        lid_clean = str(lid_raw).replace("-", "") if lid_raw != 'N/A' else 'N/A'
                        cid_clean = str(cid_raw).replace("-", "") if cid_raw != 'N/A' else 'N/A'
                        st.write(f"login_customer_id (raw): {lid_raw}")
                        st.write(f"login_customer_id (clean): {lid_clean} ({len(lid_clean)}자리)")
                        st.write(f"customer_id (raw): {cid_raw}")
                        st.write(f"customer_id (clean): {cid_clean} ({len(cid_clean)}자리)")
                        ...
                        try:
                            ...
                            # 간단한 API 호출 테스트 (accessible_customers)
                            st.markdown("---")
                            st.write("**API 접근 테스트 (accessible_customers):**")
                            try:
                                customer_service = test_client.get_service("CustomerService")
                                accessible = customer_service.list_accessible_customers()
                                st.success(f"접근 가능 계정 {len(accessible.resource_names)}개")
                                for rn in accessible.resource_names[:10]:
                                    st.write(f"  {rn}")
                            except Exception as api_err:
                                api_detail = gads._extract_google_ads_error(api_err)
                                st.error(f"API 접근 실패: {api_detail}")

                            # customer_id로 직접 쿼리 테스트
                            st.write(f"**customer_id ({cid_clean}) 직접 쿼리 테스트:**")
                            try:
                                ga_svc = test_client.get_service("GoogleAdsService")
                                test_query = "SELECT customer.id, customer.descriptive_name FROM customer LIMIT 1"
                                resp = ga_svc.search_stream(customer_id=cid_clean, query=test_query)
                                for batch in resp:
                                    for row in batch.results:
                                        st.success(f"고객 조회 성공: {row.customer.descriptive_name} (ID: {row.customer.id})")
                            except Exception as query_err:
                                query_detail = gads._extract_google_ads_error(query_err)
                                st.error(f"customer_id 쿼리 실패: {query_detail}")

                                # login_customer_id로도 테스트
                                if lid_clean != cid_clean:
                                    st.write(f"**login_customer_id ({lid_clean})로 쿼리 테스트:**")
                                    try:
                                        resp2 = ga_svc.search_stream(customer_id=lid_clean, query=test_query)
                                        for batch in resp2:
                                            for row in batch.results:
                                                st.success(f"MCC 조회 성공: {row.customer.descriptive_name} (ID: {row.customer.id})")
                                        st.info("→ login_customer_id는 접근 가능하지만 customer_id에 접근 불가. customer_id가 이 MCC 하위 계정인지 확인 필요.")
                                    except Exception as mcc_err:
                                        mcc_detail = gads._extract_google_ads_error(mcc_err)
                                        st.error(f"MCC 쿼리도 실패: {mcc_detail}")
