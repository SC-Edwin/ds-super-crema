...
│   ├── meta/facebook_ads.py   ← Meta API 핵심 (Test + 공용)
│   ├── meta/fb.py             ← Meta 마케터 모드
│   ├── unity/unity_ads.py     ← Unity API 핵심
│   ├── unity/uni.py           ← Unity 마케터 모드
│   ├── google_ads/google_ads.py
│   ├── google_ads/ga.py       ← Google Ads 마케터 UI
│   ├── mintegral/mintegral.py
│   └── applovin/applovin.py
├── utils/                     ← Drive 임포트, devtools, upload_logger
├── config/game_manager.py
├── scripts/generate_refresh_token.py
└── vietnam.py                 ← 베트남팀 전용 탭 (prefix="vn")
```
...
| `utils/video_thumbnail.extract_thumbnail_from_video` | OpenCV로 영상 중간 프레임 추출 → JPEG 저장 (`facebook_ads`에서 re-export) |
